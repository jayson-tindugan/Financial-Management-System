...
const TransactionTable = () => {
    ...
    const [showTransactionVersionModal, setShowTransactionVersionModal] =
        useState(false);
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(
                "http://localhost:8001/transaction/fetchAll"
            );
    ...
    const handleInputChange = (field, value) => {
        setFormValues((prevFormValues) => ({
            ...
            [field]: value,
            total:
                (field === "amount" ? value : prevFormValues.amount) *
                (field === "quantity" ? value : prevFormValues.quantity),
        }));
    };
    ...
    const handleUpdate = async () => {
        try {
            const response = await axios.put(
                `http://localhost:8001/transaction/update/${selectedRow.transactionId}`,
                {
    ...
    const columns = [
        ...
        },
        {
            ...
            sortable: true,
            minWidth: "100px",
            cell: (row) => formatTransactionDate(row),
        },
        {
            name: "Type",
            ...
            cell: (row) => (
                <>
                    <Button
                        variant="link"
                        size="sm"
                        onClick={() => handleOpenTransactionVersionModal(row)}
                    >
